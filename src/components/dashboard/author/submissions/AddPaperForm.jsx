...
import { 
  FaPaperPlane, FaFileUpload, FaTimes, FaSpinner, FaPlus, 
  FaTrash, FaUserPlus, FaFileAlt, FaInfoCircle, FaCheck,
  FaArrowLeft, FaArrowRight, FaLock
} from 'react-icons/fa';
...
const PAPER_TYPES = [
  "Original Research", "Review Article", "Case Study", 
  "Short Communication", "Technical Note", "Letter to Editor"
];

const SUBJECT_AREAS = [
  "Biology & Life Sciences", "Computer Science & IT", "Physics & Astronomy", 
  "Chemistry & Material Science", "Mathematics", "Engineering", 
  "Medicine & Health", "Social Sciences & Humanities", "Environmental Science"
];

const COUNTRIES = [
  "United States", "United Kingdom", "Canada", "Australia", "Germany", "France", 
  "China", "India", "Japan", "Brazil", "Pakistan", "Bangladesh", "Other"
];
...
const AddPaperForm = ({ user, onCancel, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    paperDetails: {
      ...
      subject: '',
      abstract: '',
      correspondingName: user?.firstName ? `${user.firstName} ${user.lastName}` : '',
  ...
  const removeKeyword = (index) => {
    if (formData.paperDetails.keywords.length > 1) {
      const newKeywords = formData.paperDetails.keywords.filter((_, i) => i !== index);
      setFormData(prev => ({
        ...prev,
        paperDetails: { ...prev.paperDetails, keywords: newKeywords }
      }));
  ...
  const addAuthor = () => {
    setFormData(prev => ({
      ...prev,
      authors: [...prev.authors, { fullName: '', email: '', country: '', affiliation: '' }]
    }));
  };
  ...
  const validate = () => {
    ...
    if (!paperDetails.type) newErrors['paperDetails.type'] = 'Paper type is required';
    if (!paperDetails.runningTitle) newErrors['paperDetails.runningTitle'] = 'Running title is required';
    if (!paperDetails.subject) newErrors['paperDetails.subject'] = 'Subject area is required';
    if (!paperDetails.abstract) newErrors['paperDetails.abstract'] = 'Abstract is required';
    if (!paperDetails.correspondingName) newErrors['paperDetails.correspondingName'] = 'Corresponding author name is required';
    if (!paperDetails.correspondingEmail || !/^\S+@\S+\.\S+$/.test(paperDetails.correspondingEmail)) 
      newErrors['paperDetails.correspondingEmail'] = 'Valid email is required';
    
    const validKeywords = paperDetails.keywords.filter(k => k.trim().length > 0);
    if (validKeywords.length === 0) newErrors['paperDetails.keywords'] = 'At least one keyword is required';

    // Authors
    formData.authors.forEach((author, index) => {
      if (!author.fullName || author.fullName.length < 2) 
        newErrors[`authors.${index}.fullName`] = 'Full name required (min 2 chars)';
      if (!author.email || !/^\S+@\S+\.\S+$/.test(author.email)) 
        newErrors[`authors.${index}.email`] = 'Valid email required';
      if (!author.country) newErrors[`authors.${index}.country`] = 'Country required';
      if (!author.affiliation) newErrors[`authors.${index}.affiliation`] = 'Affiliation required';
    });

    // Conflict & Data
    if (formData.conflictOfInterest && (!formData.conflictDescription || formData.conflictDescription.length < 5))
      newErrors.conflictDescription = 'Description required (min 5 chars)';
    if (!formData.dataAvailability) newErrors.dataAvailability = 'Data availability statement is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      suppFiles.forEach(file => submitData.append('supplementaryDetails', file));
      ...
      Object.keys(formData.paperDetails).forEach(key => {
        if (key === 'keywords') {
          formData.paperDetails.keywords.forEach(kw => finalData.append('paperDetails[keywords][]', kw));
        } else {
      ...
      suppFiles.forEach(file => finalData.append('supplementaryDetails', file));
  ...
  const TabButton = ({ index, label, icon: Icon }) => (
    ...
    >
      <Icon className={activeTab === index ? 'text-blue-600' : 'text-gray-400'} />
      {label}
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-black text-gray-800 tracking-tight">Paper Submission</h2>
          <p className="text-gray-500 text-sm italic">Ensure all fields marked with * are completed</p>
        </div>
        <button 
          onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="space-y-6 animate-fadeIn">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  />
                  {errors['paperDetails.title'] && <p className="mt-1.5 text-xs font-medium text-red-500 flex items-center gap-1"><FaInfoCircle /> {errors['paperDetails.title']}</p>}
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {PAPER_TYPES.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                  {errors['paperDetails.type'] && <p className="mt-1.5 text-xs font-medium text-red-500">{errors['paperDetails.type']}</p>}
                </div>

                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Running Title *</label>
                  <input 
                    type="text"
                    name="runningTitle"
                  ...
                  />
                  {errors['paperDetails.runningTitle'] && <p className="mt-1.5 text-xs font-medium text-red-500">{errors['paperDetails.runningTitle']}</p>}
                </div>

                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Subject Area *</label>
                  <select 
                    name="subject"
                  ...
                  >
                    ...
                    {SUBJECT_AREAS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                  {errors['paperDetails.subject'] && <p className="mt-1.5 text-xs font-medium text-red-500">{errors['paperDetails.subject']}</p>}
                </div>

                <div>
                   <label className="block text-sm font-bold text-gray-700 mb-2">Corresponding Author Email *</label>
                   ...
                   <p className="mt-1 text-[10px] text-gray-400">Pre-filled from your profile</p>
                </div>

                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Abstract *</label>
                  <textarea 
                    name="abstract"
                    rows="6"
                  ...
                  ></textarea>
                  {errors['paperDetails.abstract'] && <p className="mt-1.5 text-xs font-medium text-red-500">{errors['paperDetails.abstract']}</p>}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  {errors['paperDetails.keywords'] && <p className="mt-1.5 text-xs font-medium text-red-500">{errors['paperDetails.keywords']}</p>}
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 1 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-gray-800">Add Co-Authors</h3>
                <button 
                  type="button" 
                  onClick={addAuthor}
              ...
              {formData.authors.map((author, idx) => (
                <div key={idx} className="p-6 bg-gray-50/50 rounded-2xl border border-gray-100 relative group">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-gray-500 mb-1 block uppercase">Full Name *</label>
                      <input 
                        type="text"
                        value={author.fullName}
                        onChange={(e) => handleAuthorChange(idx, 'fullName', e.target.value)}
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => handleAuthorChange(idx, 'country', e.target.value)}
                    ...
                    <div>
                      <label className="text-xs font-bold text-gray-500 mb-1 block uppercase">Affiliation *</label>
                      <input 
                        type="text"
              ...
              <div className="bg-yellow-50/50 p-6 rounded-2xl border border-yellow-100">
                <div className="flex items-start gap-4 mb-4">
                  <div className="mt-1">
                    <input 
                      ...
                      onChange={(e) => setFormData(p => ({ ...p, conflictOfInterest: e.target.checked }))}
                    />
                  </div>
                  <label htmlFor="conflict" className="cursor-pointer">
                    <span className="block font-bold text-gray-800">Conflict of Interest Disclosure</span>
                    <span className="text-sm text-gray-600 leading-relaxed">
                      Check this box if authors have any financial or personal relationships that could inappropriately 
                      influence (bias) their actions or research findings.
                    </span>
                  </label>
                </div>
                ...
                {formData.conflictOfInterest && (
                  <div className="mt-4 animate-slideDown">
                    <label className="block text-sm font-bold text-gray-700 mb-2">Please describe the conflict *</label>
                    <textarea 
                      rows="4"
                      ...
                      onChange={(e) => setFormData(p => ({ ...p, conflictDescription: e.target.value }))}
                    ></textarea>
                     {errors.conflictDescription && <p className="mt-1.5 text-xs font-medium text-red-500">{errors.conflictDescription}</p>}
                  </div>
                )}
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Data Availability Statement *</label>
                <textarea 
                  rows="4"
                  ...
                  onChange={(e) => setFormData(p => ({ ...p, dataAvailability: e.target.value }))}
                ></textarea>
                {errors.dataAvailability && <p className="mt-1.5 text-xs font-medium text-red-500">{errors.dataAvailability}</p>}
              </div>
            </div>
          )}
          ...
          {activeTab === 3 && (
            <div className="space-y-8 animate-fadeIn">
              {/* Paper File Upload */}
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-3">Main Research Paper (PDF) *</label>
                <div className={`border-2 border-dashed rounded-2xl p-10 text-center transition-all ${paperFile ? 'border-green-300 bg-green-50' : errors.paper ? 'border-red-300 bg-red-50' : 'border-gray-200 hover:border-blue-400 group'}`}>
                  {paperFile ? (
                    <div className="flex flex-col items-center">
                      <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-4">
                        <FaCheck size={30} />
                      </div>
                      <p className="font-bold text-green-900">{paperFile.name}</p>
                      <p className="text-xs text-green-600/70">{(paperFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                      <button 
                        type="button"
                    ...
                    <div className="flex flex-col items-center cursor-pointer" onClick={() => document.getElementById('paperFile').click()}>
                      ...
                      <p className="font-bold text-gray-700">Click to upload paper</p>
                      <p className="text-xs text-gray-400 mt-1">PDF format only (Max 20MB)</p>
                      <input type="file" id="paperFile" className="hidden" accept=".pdf" onChange={(e) => handleFileChange(e, 'paper')} />
                    </div>
                  )}
                </div>
                {errors.paper && <p className="mt-2 text-center text-xs font-medium text-red-500">{errors.paper}</p>}
              </div>

              {/* Multiple Sections for Figures and Supp */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-3">Figures & Illustrations (Optional)</label>
                  <div className="space-y-3">
                    <button 
                      type="button"
                      onClick={() => document.getElementById('figFiles').click()}
                    ...
                    <input type="file" id="figFiles" className="hidden" multiple onChange={(e) => handleFileChange(e, 'figure')} />
                    
                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                      {figFiles.map((file, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg text-xs truncate">
                          ...
                          <button onClick={() => removeFile(idx, 'figure')} className="text-gray-300 hover:text-red-500"><FaTrash /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-3">Supplementary Materials (Optional)</label>
                  <div className="space-y-3">
                    <button 
                      type="button"
                      onClick={() => document.getElementById('suppFiles').click()}
                      className="w-full p-4 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-all text-sm font-bold"
                    >
                      + Add Supplementary
                    </button>
                    <input type="file" id="suppFiles" className="hidden" multiple onChange={(e) => handleFileChange(e, 'supplementary')} />
                    ...
                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                      {suppFiles.map((file, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg text-xs truncate">
                          ...
                          <button onClick={() => removeFile(idx, 'supplementary')} className="text-gray-300 hover:text-red-500"><FaTrash /></button>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="flex items-center justify-between pt-10 mt-10 border-t border-gray-100">
            ...
            <div className="flex items-center gap-4">
              {activeTab < 3 ? (
                ...
                >
                  {loading ? <FaSpinner className="animate-spin text-lg" /> : <FaPaperPlane className="text-base" />}
                  Submit Manuscript
                </button>
              )}
            </div>
